...
use Royalcms\Component\Gettext\Config\Models\Config;
use Royalcms\Component\Gettext\Exceptions\LocaleFileNotFoundException;
use Royalcms\Component\Gettext\Exceptions\FileCreationException;
...
class FileSystem {
    ...
    public function compileViews(Array $viewPaths, $domain)
    {
        ...
        foreach ( $viewPaths as $path ) {
            ...
            $fs = new \Royalcms\Component\Filesystem\Filesystem($path);
            $glob = $fs->glob(realpath($path) . '/{,**/}*.php', GLOB_BRACE);
            $compiler = new \Royalcms\Component\View\Compilers\BladeCompiler($fs, $domainDir);
